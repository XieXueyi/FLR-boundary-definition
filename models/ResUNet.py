...
class ResUNet(nn.Module):
    def __init__(self, training=True):
        ...
        self.encoder1 = nn.Sequential(  # 两次卷积
            ...
            nn.Conv2d(16, 16, 3, 1, 1),  # 3D卷积的kernel_size为(3,3,3)，对应(D,H,W)，对每个channel进行卷积
            nn.PReLU(16)
        )  # [1, 16, 16, 256, 256]
        ...
        self.output4 = nn.Sequential(  # decoder2的输出
            nn.Conv2d(32, 1, 1, 1),
            # nn.Upsample(scale_factor=(1, 1), mode='trilinear'),
            nn.Sigmoid()
        )  # [1, 1, 16, 256, 256]
...
#         if self.training:  # 训练阶段，输出四组output，测试阶段，只输出output4即可
...
if __name__ == "__main__":
    ...
    tensor_rand = torch.rand(3, 1, 512, 512).to('cuda')
    model = ResUNet().to('cuda')
    pred = model(tensor_rand)
    print('model')